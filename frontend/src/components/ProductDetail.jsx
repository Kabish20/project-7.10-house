...
const ProductDetail = () => {
  const { activeHeroProduct, setActiveView, addToCart, setCartOpen, formatPrice, API_URL } = useShop();
  ...
  const handleWhatsAppEnquire = async () => {
    ...
    if (includeImage && base64Images.length > 0) {
      try {
    ...
    const imageLinksText = includeImage ? resolvedUrls.map((url, idx) => {
